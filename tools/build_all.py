...
#
# 라벨 자리를 두 번 옮겼다.
#   본문 여유 슬롯(185..190) -> 실패. 본문 슬롯은 곧 **가나 폰트 자리**이고 상태창과
#     같이 보이는 지휘관 이름판이 원본 카타카나다. `ル`=185 `レ`=186 `ン`=189 이
#     우리 `지` `휘` `수` 로 변해 이름판이 `휘ディ수` 가 됐다.
#   팝업 칸(206..211) -> 실패. 팝업이 상태창과 **같이 뜬다.** "MP가 모자랍니다" 나
#     전직 팝업이 뜨면 그 글리프가 라벨을 덮어 `지휘범위 수정` 이 `인지` 로 변한다
#     (강제 표시 실측). "다시 그리면 낫는다" 로 넘길 문제가 아니었다.
#   지금은 **전용 페이지 5, 타일 245..250** 이다. 이름(192..197)·클래스명(198..205)
#     ·팝업(206..220)·용병(221..244) 어느 것과도 겹치지 않는다.
...
#   0x0158DA 는 초상 + 이름 (대사 이름판 변형), 0x020BB2 는 이름 + 클래스명 창.
#   둘 다 한 번에 하나만 보이므로 위치 코드(타일 192..197)로 충분하다.
...
#   1 용병 클래스     타일 221.. — 팝업(206..220)과 겹치지 않게 뒤로 물렸다
...
#   5 상태창 라벨              타일 236.. — 팝업(206..220)·용병(212..235)·클래스명
#                            (198..205)·이름(192..197)과 모두 안 겹치는 유일한 자리
# 전투 화면에 동시에 보이는 것들이 타일 192..255 를 나눠 쓴다. 64칸을 이렇게 갈랐다.
#   192..197  이름 6        198..205  클래스명 8 (위치 코드)
#   206..220  팝업 단문 15   221..244  용병 클래스 어휘 24 (페이지 1)
#   245..250  상태창 라벨 6  251..255  여유
#   6 아래 상태바 이름  타일 119..122 — 대사 이름판(192..197)과 같이 보이므로 따로.
#     처음에 124..127 을 썼다가 실패했다. **대사창의 "계속" 프롬프트가 타일 124/125 를
#     스프라이트로 번갈아 그린다**(0x0D324: 프레임 카운터 비트로 807C/807D 를 고른다).
#     그래서 대사창 오른쪽 아래에 `레`/`딘` 이 0.5초마다 번갈아 보였다.
#     빈 타일 판정을 네임테이블 덤프만으로 했던 것이 원인이다 — 스프라이트로 그리는
#     타일은 네임테이블에 안 나온다. 이번에는 **코드의 타일 워드 즉치도 전수로** 훑어
#     스프라이트 사용 타일(96/98/101/106/107/123/124/125/126)을 확인했고,
#     119..122 는 덤프에도 코드에도 없다.
# (번호, 시작 타일, 재매핑할 코드 목록 이름)
PAGES = {"merc": (1, 221, "body"), "cand": (2, 214, "body"), "cond": (3, 192, "body"),
         "stage": (4, 232, "body"), "panel": (5, 245, "body"), "bar": (6, 119, "name")}
...
#   0x00C486 은 **아래 상태바**다 — 0xC438 이 클래스명($E2F8)을 그린 뒤 같은 줄에
...
#   0x01083A  유닛 목록 ($E698 배열 루프, 그리기 $60BE -> $5F60)
#   0x0114DE  출전 준비 지휘관 목록 창 (2열 x 6줄)
#   0x013590  지휘관 목록 창 (다른 진입, $FF3004 루프)
#   0x012B18  유닛 목록 (또 다른 진입, $E690 기준)
#   0x0140DC  출전 준비 위·아래 지휘관 카드 8개 — **플레인 B**($E000) 에 그린다.
#             처음에 0x01083A 를 카드로 오인했다. 카드가 원문 셀 그대로 남고 목록이
#             올린 우리 글리프를 가리켜 `レ휘크ン` 처럼 섞여 보인 것이 실측 증거다
#             (타일 185 이상은 우리가 안 덮으므로 끝 글자만 가나로 남았다).
NAMEPTR_ROSTER = (0x01083A, 0x0114DE, 0x013590, 0x012B18, 0x0140DC)
BAR_CELLS = 4
    ...
    if shared_strs:
        ...
        if pbase + len(vocab) > 256:
            raise SystemExit(f"상태창 라벨 어휘 {len(vocab)}자가 페이지 타일 "
                             f"{256 - pbase}칸을 넘는다")
        shared_code = {ch: CODES[i] for i, ch in enumerate(vocab)}
        shared_rk = len(recs)
        recs.append((pbase, vocab, ppage))
...
def main() -> None:
    ...
    # 대상 선택 라벨 4개 — 리소스 0x83 의 픽셀을 제자리에서 갈아 끼운다. 훅 없음.
    # 넷이 같은 타일 362..381 을 돌려쓰므로 데이터를 갈아야 모드마다 다 덮인다.
    targets, attack_log = menu.build_target_labels(src)
    for at, blob in targets:
    ...
    menu_log += (["  -- 선택 창 제목 (풀 0x81)"] + title_log
                 ...
                 + ["  -- 대상 선택 라벨 (리소스 0x83 데이터 교체)"] + attack_log)
    ...
    for name, (idx, base, which) in sorted(PAGES.items(), key=lambda kv: kv[1][0]):
        ...
        codes = CODES if which == "body" else NAME_CODES[:BAR_CELLS]
        # 이름 페이지는 **BAR_CELLS 개만** 재매핑한다. 6개를 다 옮기면 뒤 두 칸이
        # 타일 123/124(스프라이트 사용)로 가리켜 잠재적 함정이 된다.
        for i, c in enumerate(codes):
            if base + i < 256:
                tbl[c] = base + i
        pages += tbl
