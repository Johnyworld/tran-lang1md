...
# --------------------------------------------------------------- 전투씬 병력 바
# 전투 애니메이션 화면 양쪽 패널의 병력 바. 텍스트가 아니라 **그래픽**인데 우리
# 글리프 슬롯과 자리를 다툰다. 실기에서 왼쪽(적) 바 안에 `적적` 이 보였다.
#
# ```
# 0x0D1C2  move.w #$807e, d1        바 타일 베이스
# 0x0D1C6  d1 += 0x40 * d7          변형 0 = 126.. / 변형 1 = 190..
#          base+0 가득 / base+1 부분 / base+2 빈칸
# ```
# 즉 **빈칸 타일이 128(변형 0)과 192(변형 1)** 이고 그 둘이 정확히 우리 본문 슬롯
# 첫 칸과 이름 슬롯 첫 칸이다. 적의 병력이 0 이면 바 전체가 빈칸 타일이므로 대사에서
# 올라간 글리프가 그대로 늘어서 보인다.
#
# 빈칸 그림은 테두리만 있고 속은 색 E 다 — 원본 타일 127(변형 0 의 부분 타일)이
# 그 모양 그대로이고, 변형 1 의 부분 타일 191 도 채운 1px 을 빼면 같다. 그래서
# 그 32바이트를 데이터로 들고 있다가 **바를 그릴 때마다 128·192 에 되돌린다.**
# 전투씬에는 한글이 없으므로 글리프를 잃어도 되고, 맵으로 돌아가면 문자열이 다시
# 그려질 때 우리 업로더가 글리프를 되올린다(스스로 낫는 성질).
BAR_EMPTY_TILES = (128, 192)
BAR_HOOK = 0xD18C
BAR_STEAL = bytes.fromhex("223C80658065")        # move.l #$80658065, d1
BAR_EMPTY_ROWS = ("DDDDDDDD", "77777777", "EEEEEEEE", "EEEEEEEE",
                  "EEEEEEEE", "EEEEEEEE", "33333333", "EEEEEEEE")


def bar_empty_tile() -> bytes:
    """빈칸 바 타일 32바이트 (4bpp)."""
    out = bytearray()
    for row in BAR_EMPTY_ROWS:
        for i in range(0, 8, 2):
            out.append((int(row[i], 16) << 4) | int(row[i + 1], 16))
    assert len(out) == 32
    return bytes(out)
